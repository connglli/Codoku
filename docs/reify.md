...
The guard fires only on states the twin is right for, so wherever it fires the twin produces exactly what the region would and everywhere else the original runs — hence full equivalence on every input. The guard is a generated function `@__twg_<fn>_<label> : i1`, one per twin site. Scalar roots cross into the guard by value, vector roots per-lane, and aggregate roots by address (`ptr [N] T` / `ptr @S` parameters, navigated inside the guard with in-bounds `ptrindex`/`ptrfield` + `load`). Every form it can take — equality, a pair of bounds, a pointer comparison — is total, so the guard never traps on any input. Each candidate region is twinned with probability `--p-twin`.

**What the guard says about each leaf.** A trace body is right for every state that follows the same path UB-free, which is far more than the one state it was profiled on — so the guard is not a conjunction of equalities over the whole state. An interval pass over the flattened body (`reify/twin_interval.hpp`) decides, per integer leaf:

| class | guard says | established by |
|---|---|---|
| **free** | nothing — the leaf is not compared, not navigated to, and not even a parameter | the trace is provable with the leaf *unknown*, which covers every value it could hold |
| **ranged** | `lo <= x <= hi` | the widest run the pass can prove, found by doubling each open leaf's radius in lockstep and settling the last step by bisection |
| **pinned** | `x == v` | nothing wider was provable |

A refused round freezes only the leaves whose failing check depended on them, so no leaf's width depends on the order the search happened to visit it. Lockstep still ties leaves together, though — one that is amplified before a check reaches its own limit and freezes the others with it — so afterwards each leaf is grown again *alone*, with the rest held where lockstep left them; that needs no argument about whose fault a refusal was, since every step of it is proved like any other. Ranges are clipped to the leaf's own width (a leaf holds no value its type cannot represent) and trimmed by a seeded amount so a region does not always produce the identical guard. Floats and pointers stay pinned: bounding a float needs rounding-aware arithmetic, and a pointer has no range to speak of. On a counting loop this comes out as *shape pinned, data free* — the trip count and induction variable pin themselves, because moving either takes a different path, while the accumulator opens over millions of values.

Because the guard now admits far more than one state, `--validate` no longer covers it by running the profiled input alone: it also samples states inside each box — corners first — and runs the region against the twin body on each, reporting how many agreed.
...
A trace body is also correct on far more than the profiled state: it reproduces the region's effect for **every** state that follows the same path UB-free. That is the strength the guard box above spends: the box is exactly the part of it the interval pass can prove.

**The body is rewritten before it is grafted.** A flattened trace is the region's own statements back in order, which is correct and entirely recognizable — so an *anti-optimization* engine (`reify/antiopt.hpp`) rewrites it by identities that preserve what it computes and change how it says it. The direction is the point: a compiler rewrites toward a normal form, and a twin that normalizes back to its region is one any optimizer sees through, so every rule moves the other way. Today's catalog crosses between the arithmetic and bitwise domains — `x ^ y` as `(x | y) - (x & y)`, `x & y` as `x - (x & ~y)`, `x | y` as `x + y - (x & y)`, the carry identity for `+` and its dual for `-`, De Morgan in the expanding direction, and splitting a non-negative value into its high and low halves — runs the standard peepholes backwards (a subtraction spelled as an addition of a complement, a literal split into two that sum to it, a subtracted literal respelled as an added negative one, a doubling turned into a multiplication, a complement into `0 - x - 1`, a multiplication by a power of two into a shift, a comparison with its operands swapped, a value round-tripped through a wider type), inserts cancelling `^` pairs, and restructures — breaking a chain apart through a fresh temporary, reordering statements that do not depend on each other, recomputing a value instead of reusing the temp that holds it, padding the body with dead statements drawn from rysmith's own expression generator so they read as native, and routing a value through a scratch cell so an optimizer has to alias-analyse what it used to read straight off the dataflow. The body is straight-line, so what stands in for control flow is the *choosing*: a select becomes the branchless `b + ((a - b) & (c as iN))` — `i1` true is all-ones, so the cast is a ready-made mask — a relation becomes two weaker ones combined with `&` or `|`, a comparison moves into arithmetic as `a - b < 0`, and an intrinsic is written out as the select or the pair of comparisons it stands for, which removes the name a reader was going to match on. A rule that introduces a trapping operation is *offered* everywhere and *kept* only where the box proves it safe: `<<` is signed arithmetic in RefractIR and traps on a negative left operand, so a multiplication becomes a shift exactly where the guard holds the multiplicand above zero.

One family is unlike the rest. Every rule above is an identity for all inputs; the *licensed* rules are identities only over the states the guard admits, and they read the interval table that certified it to find out where — a literal spelled as a read of a local the table pins to that value, a mask that is a no-op because the value cannot be larger, a select whose other arm never runs because the table settles its condition, a constant produced by shifting a leaf whose whole admitted range lands on one number, and a cancelling pair keyed on a leaf the guard does not mention at all. No generic obfuscator can write these, because they depend on facts the guard is enforcing; the engine takes them through an abstract `AntiOptFacts`, so a caller with nothing to say (rysmith, rylink) simply never fires them. Since a licence granted on a body that has since changed is no licence, the facts are recomputed before every attempt rather than once per round.

The engine scans for candidates once per *attempt* rather than once per round: a candidate list outlives its body by one rewrite, since a splice moves every position after it, and with a catalog this size a half-invalidated list hands most of its picks to whatever sits early in the body.

Rule-level soundness says nothing about two rules together — each may be value-preserving while the pair puts an intermediate outside its type — so after every application the interval pass re-checks the whole body against **every state the guard admits**, and a body that no longer proves is rolled back. That is what makes the catalog safe to grow: a wrong rule costs a rewrite, never correctness. Where the pass cannot prove the trace at all — an intrinsic result feeding arithmetic is enough — there is nothing to judge rewrites against, and the body would otherwise be grafted exactly as the region wrote it, which is the one case a twin must not be. What a proof buys is permission to introduce operations that *can* trap, so in that case the trap-free rules still apply (they are identities whatever the state, and so is any composition of them) and the rest stand down; the graft log says `trap-free only` when this happens. Each rule also nominates an example it fires on, and the hidden `rytwin --selftest-antiopt` settles the claim by *running* both spellings through the interpreter over every `i8` value of the operands they read, comparing the resulting state and the UB outcome. The interpreter is the authority on what a statement computes, so the check asks it rather than restating the arithmetic — a restatement only ever checks itself, and cannot check a rule with no closed form at all, which was half the catalog. The tier declaration is checked with it: a trap-free rule that turns a UB-free run into a trapping one is a failure, while a rule that may trap is compared only where both runs are clean, which is the permission the caller's proof buys it. Proving the identities outright, at real widths rather than over an `i8` sweep, is what a translation-validation tool would be for.

**rylink runs the same catalog over its bundles.** A whole program assembled from one statement generator's output has the same problem a trace body does, from the other direction: its functions read alike. `AntiOptTransform` is the catalog as a whole-program `Transform`, added after call realization so realization still sees the code the generator wrote. It passes no acceptance predicate — a concrete program has no set of states to prove anything over — so the engine offers only the trap-free rules, which need no proof. `--no-antiopt` turns it off.
