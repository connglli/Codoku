...
namespace refractir {
  ...
  namespace {
    ...
    public:
      Checker(
          const FunDecl &fn, const TypeUtils::StructTable &structs, const EntryState &entry,
          bool record = false, const IntrinsicFold *fold = nullptr
      ) :
      ...
      run(const std::vector<Instr> &stmts, const std::vector<BranchObligation> &branches) {
        ...
        for (std::size_t i = 0; i < stmts.size(); ++i) {
          ...
          // An obligation placed *before* this statement sees the state the
          // caller saw when it decided the branch.
          while (next < branches.size() && branches[next].afterStmt <= i) {
    ...
    private:
      ...
      // Rejects a dereference the body could not perform safely.
      ...
      // tracked. `bad` is set when the body could not be proven safe, which a
      ...
      std::optional<Interval> evalCoef(const Coef &c, std::uint32_t bits) {
        ...
        if (auto id = std::get_if<LocalOrSymId>(&c)) {
          ...
          if (!loc)
            return unknownOf(); // a symbol has no value here
          LValue lv;
          lv.base = *loc;
          if (widthOf(lv) == 0)
            return unknownOf();
      ...
      std::optional<Interval> evalOp(const OpAtom &o, std::uint32_t bits) {
        ...
        switch (o.op) {
          ...
          case AtomOpKind::Shl: {
            ...
            // both would claim a narrower result than the body can produce,
      ...
      bool decides(const BranchObligation &chk) {
      ...
      // Assigning a float: an unproven *operation* stops the pass, while a
      ...
      PtrEnv ptrs_;
      const TypeUtils::StructTable &structs_;
      // What an intrinsic call computes when its arguments are single values,
      // if the caller can say. See IntrinsicFold.
  ...
  std::vector<IntervalEnv> IntervalAnalysis::snapshots(
    ...
    chk.run(stmts, branches); // a body that fails still recorded what it got
  ...
  namespace {
    ...
    // A requirement on a value: where it must lie for the body to run as the
    ...
    // The value of a literal or a bare local at one point of the body.
  ...
  std::unordered_map<std::string, Interval> IntervalAnalysis::ceilings(
    ...
    std::size_t check = branches.size();
    for (std::size_t i = stmts.size(); i-- > 0;) {
      ...
      while (check > 0 && branches[check - 1].afterStmt > i) {
        --check;
        const BranchObligation &c = branches[check];
      ...
      auto *ai = std::get_if<AssignInstr>(&stmts[i]);
