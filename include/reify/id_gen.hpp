...
// Shared 6-hex-char generation-ID helper used by rysmith and
...
// across tools: consumers of the ID can re-use the same
...
namespace refractir::reify {
  ...
  [[nodiscard]] inline std::string genHexId(std::mt19937 &rng) {
    std::uniform_int_distribution<std::uint32_t> d(0, 0xFFFFFFu);
    char buf[8];
